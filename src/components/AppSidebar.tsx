...
function SectionLabel({ open, children }: { open: boolean; children: React.ReactNode }) {
  if (!open) return null;
  return <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground px-3 mb-1">{children}</p>;
}

function SidebarLink({ item, open, active }: { item: typeof navItems[0]; open: boolean; active: boolean }) {
  return (
    <NavLink
      to={item.path}
